...
func TestDayParser_Parse(t *testing.T) {
	...
	tbl := []struct {
		...
		{time.Date(2018, 01, 15, 14, 40, 0, 0, nytz), "xxx {{.MM}} blah {{.DD}}", "xxx 01 blah 15", nil},
		{time.Date(2018, 01, 15, 14, 40, 22, 123000000, nytz), "xxx {{.UNIX}} blah {{.UNIXMSEC}}", "xxx 1516045222 blah 1516045222123", nil},
		{time.Date(2018, 01, 15, 14, 40, 0, 0, nytz), "{{.MMXX}}", "", nil},
		{time.Date(2018, 01, 15, 14, 40, 0, 0, nytz), "zz {{.YYMMDD}}", "zz 180115", nil},
		{time.Date(2018, 01, 15, 14, 40, 0, 0, nytz), "zz {{.YY}}", "zz 18", nil},
	}
...
func TestDayParser_weekdayBackward(t *testing.T) {
	...
	tbl := []struct {
		...
		{nil, time.Date(2017, 04, 30, 0, 0, 0, 0, nytz), time.Date(2017, 04, 28, 0, 0, 0, 0, nytz)}, // sun
		{nil, time.Date(2017, 04, 29, 0, 0, 0, 0, nytz), time.Date(2017, 04, 28, 0, 0, 0, 0, nytz)}, // sat

		{[]time.Weekday{time.Saturday}, time.Date(2017, 04, 30, 0, 0, 0, 0, nytz), time.Date(2017, 04, 30, 0, 0, 0, 0, nytz)}, // sun
		{[]time.Weekday{time.Saturday}, time.Date(2017, 04, 29, 0, 0, 0, 0, nytz), time.Date(2017, 04, 28, 0, 0, 0, 0, nytz)}, // sat
	}
...
func TestDayParser_weekend(t *testing.T) {
	...
	tbl := []struct {
		...
		{time.Date(2017, 04, 30, 0, 0, 0, 0, nytz), "{{.WYYYYMMDD}} {{.YYYYMMDD}}", "20170428 20170430"}, // sun
		{time.Date(2017, 04, 29, 0, 0, 0, 0, nytz), "{{.WYYYYMMDD}} {{.YYYYMMDD}}", "20170428 20170429"}, // sat
	}
...
func TestDayParser_holiday(t *testing.T) {
	...
	d = NewDayTemplate(time.Date(2020, 7, 16, 18, 0, 0, 0, nytz), TimeZone(nytz), Holiday(HolidayCheckerFunc(func(day time.Time) bool {
		return day.After(time.Date(2020, 7, 10, 00, 0, 0, 0, nytz))
	})))
